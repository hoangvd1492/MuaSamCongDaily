...
def is_task_pending(task_id: str | int) -> bool:
    """Kiểm tra task dựa trên ID thay vì ma_tbmt."""
...
async def add_download_task(
    ...
    """Thêm task tải HSMT vào hàng đợi, dùng ID để xác định."""
    if is_task_pending(id):
        return False

    PENDING_TASKS.add(id)
    task_payload = {
        "id": id,
        "ma_tbmt": ma_tbmt,
        "chat_id": chat_id,
    ...
    logger.info(f"Đã thêm gói thầu ID: {id} ({ma_tbmt}) vào hàng đợi.")
    return True


async def hsmt_download_worker(bot):
    """Worker chạy nền liên tục lấy task từ Queue để xử lý và lưu file."""
    ...
    while True:
        ...
        try:
            ...
            # 4. Đọc file từ ổ đĩa và gửi qua Telegram
